...
public class OtterEntity extends Animal {
    ...
    protected void registerGoals() {
        this.goalSelector.addGoal(0, new AvoidEntityGoal<>(this, Player.class, 32.0F, 0.9D, 1.5D,
                (livingEntity -> livingEntity.equals(this.getLastHurtMob()))));
        ...
        this.targetSelector.addGoal(1, new NearestAttackableTargetGoal<>(this, AbstractFish.class, 20, false, false,
                (fish) -> fish instanceof AbstractSchoolingFish && this.getHuntDelay() <= 0));
    }
        ...
        if (this.isAlive() && this.isEffectiveAi()) {
            ...
            if (this.isEating()) {
                if (this.eatDelay > 0) {
                    ...
                    if (--this.eatTime <= 0) {
                        this.consumeItem(this.level(), this.getMainHandItem());
    ...
    public ItemStack consumeItem(Level level, ItemStack itemStack) {
        if (itemStack.is(ZAMItems.GEODE.get())) {
            if (this.random.nextFloat() <= 0.07F) {
                Vec3 mouthPos = this.calculateMouthPos();
                ItemEntity pearl = new ItemEntity(level, mouthPos.x(), mouthPos.y(), mouthPos.z(),
                        new ItemStack(ZAMItems.GEODE.get()));

                pearl.setDeltaMovement(
                        this.getRandom().nextGaussian() * 0.05D,
                        this.getRandom().nextGaussian() * 0.05D + 0.2D,
                        this.getRandom().nextGaussian() * 0.05D
                );
                level.addFreshEntity(pearl);
            }
            level.playSound(null, this.getX(), this.getY(), this.getZ(),
                    SoundEvents.TURTLE_EGG_BREAK, SoundSource.NEUTRAL, 0.8F, 1.5F);
            itemStack.shrink(1);
            return itemStack;
        } else {
            return itemStack;
    ...
    protected void jumpInLiquid(TagKey<Fluid> fluidTag) {
        this.setDeltaMovement(this.getDeltaMovement().add(0.0D,
                (double) 0.08F * Objects.requireNonNull(this.getAttribute(Attributes.WATER_MOVEMENT_EFFICIENCY)).getValue(),
                0.0D));
    }
    ...
    public boolean isFood(ItemStack stack) {
        return (stack.is(ItemTags.FISHES)) || stack.is(ZAMItems.GEODE.get());
    }
    ...
    public AgeableMob getBreedOffspring(ServerLevel level, AgeableMob ageableMob) {
        OtterEntity otter = ZAMEntities.OTTER.get().create(level);
        return otter;
    }
    ...
    public SpawnGroupData finalizeSpawn(ServerLevelAccessor level,
        ...
        if (spawnType.equals(MobSpawnType.SPAWNER) && this.random.nextFloat() <= 0.2F) {
    ...
    public boolean isHungryAt(ItemStack foodStack) {
        return foodStack.is(ZAMItems.GEODE.get()) || this.getInLoveTime() <= 0;
    }
    ...
    private void startEating() {
        if (this.isFood(this.getMainHandItem())) {
            this.eatDelay = this.getMainHandItem().is(ZAMItems.GEODE.get()) ? 35 : 12;
            this.eatTime = 20;
            this.setEating(true);
        }
    }
    ...
    public Vec3 calculateMouthPos() {
        Vec3 viewVector = this.getViewVector(0.0F)
                .scale(this.isFloating() ? 0.3D : 0.6D)
                .add(0.0D, this.isFloating() ? 0.55D : 0.0D, 0.0D)
                .scale(this.getScale());
        return new Vec3(this.getX() + viewVector.x(),
                this.getY() + viewVector.y(),
                this.getZ() + viewVector.z());
    }
        ...
        public boolean isStableDestination(BlockPos destination) {
            if (this.otter.isInWater() && this.level.getBlockState(destination).isAir()) {
                return !(this.level.getBlockState(destination.below()).isAir() ||
                        this.level.getBlockState(destination.below()).getFluidState().is(FluidTags.WATER));
            } else {
                return !this.level.getBlockState(destination.below()).isAir();
            }
        }
    ...
    public class GoToSurfaceGoal extends Goal {
        ...
        private void searchTargetPos() {
            if (OtterEntity.this.getMainHandItem().is(ZAMItems.GEODE.get())) {
        ...
        public void tick() {
            if (this.targetPos == null || !OtterEntity.this.level().getBlockState(BlockPos.containing(this.targetPos)).isAir()) {
                ...
                if (this.goingLand) {
                    ...
                    if (this.targetPos.y() > OtterEntity.this.getY() &&
                            targetPos.distanceToSqr(OtterEntity.this.position()) <= 3.0D) {
        ...
        public void tickTimeout() {
            if (this.timeoutTimer % 2 == 0) {
                ((ServerLevel) OtterEntity.this.level()).sendParticles(
                        ParticleTypes.BUBBLE,
                        OtterEntity.this.getRandomX(0.6D),
                        OtterEntity.this.getY(),
                        OtterEntity.this.getRandomZ(0.6D),
                        2, 0.0D, 0.1D, 0.0D, 0.0D
                );
            }
            if (this.timeoutTimer <= 0) {
                OtterEntity.this.playSound(ZAMSounds.OTTER_AMBIENT.get(), OtterEntity.this.getSoundVolume(), 0.3F);
        ...
        private Vec3 findAirPosition() {
            Iterable<BlockPos> blocksInRadius = BlockPos.betweenClosed(
                    Mth.floor(OtterEntity.this.getX() - 1.0D),
                    OtterEntity.this.getBlockY(),
                    Mth.floor(OtterEntity.this.getZ() - 1.0D),
                    Mth.floor(OtterEntity.this.getX() + 1.0D),
                    Mth.floor(OtterEntity.this.getY() + 16.0D),
                    Mth.floor(OtterEntity.this.getZ() + 1.0D)
            );
    ...
    public class SearchFoodGoal extends Goal {
        ...
        public boolean canUse() {
            if (!OtterEntity.this.getMainHandItem().isEmpty()) {
                ...
                List<ItemEntity> itemsInRadius = OtterEntity.this.level().getEntitiesOfClass(
                        ItemEntity.class,
                        OtterEntity.this.getBoundingBox().inflate(8.0D, 8.0D, 8.0D),
                        (itemEntity -> OtterEntity.this.wantsToPickUp(itemEntity.getItem()) &&
                                !OtterEntity.this.rejectedItem(itemEntity))
                );
